...
from models.model2D import GNNDecoder, GNN, GNNComplete, Multi_Motif_Fusion
...
def save_model(save_best):
    if not args.output_model_dir == '':
        if save_best:
            ...
            torch.save(model.state_dict(), path + 'regression_T_{}_contrast_{}.pth'.format(args.T, args.contrast_epochs))
...
def contrast_loss(batch_motif, batch, model, att_model):
    '''
    ...
    motif_repr, coeff = att_model(out)
...
def train(args, model_list, device, loaders, optimizer_list, contrast=False):   
    ...
    optimizer_model, optimizer_dec_pred_atoms, optimizer_attn = optimizer_list
    model.train()
    ...
    for step, (batch_motif, batch_mol) in enumerate(l): 
        ...
        if contrast: 
            ...
            con_loss = 0.2 * contrast_loss(batch_motif, batch_mol, model, att_model)
...
if __name__ == '__main__':
    ...
    torch.manual_seed(args.seed)
    loader_motif = DataLoaderMaskingPred(dataset_motif, batch_size=args.batch_size, shuffle=True, 
                                   num_workers = args.num_workers)
    torch.manual_seed(args.seed)                               
    loader = DataLoaderMaskingPred(dataset, batch_size=args.batch_size, shuffle=True, 
                                    num_workers = args.num_workers)
    NUM_NODE_ATTR = 119
    
    # === 2D Encoder Building ===  
    model = GNNComplete(num_layer=args.num_layer, emb_dim=args.emb_dim).to(device)
    ...
    optimizer_attn = optim.Adam(att_model.parameters(), lr=args.lr, weight_decay=args.decay)
    optimizer_list = [optimizer_model, optimizer_dec_pred_atoms, optimizer_attn]
    optimal_loss = 1e10
