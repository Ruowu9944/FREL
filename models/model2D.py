...
num_bond_type = 6  # including self-loop edge, and extra masked tokens
num_bond_direction = 3

# ========= Regression Module ========
from ogb.utils.features import get_atom_feature_dims
full_atom_feature_dims = get_atom_feature_dims()
full_atom_feature_dims[0] += 1 # Including the extra mask tokens
...
class GNN(nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0., gnn_type="GIN"):
        ...
        for layer in range(num_layer):
            if gnn_type == "GIN":
                self.gnns.append(GINConv(emb_dim, aggr = "mean")) # default aggregation is 'add'
            elif gnn_type == "GCN":
...
class GINConvComplete(MessagePassing):
    def __init__(self, emb_dim, aggr="add"):
        ...
        '''
        super(GINConvComplete, self).__init__(aggr=aggr)
...
class GNNComplete(nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, JK="last", drop_ratio=0., gnn_type="gin"):
        ...
        for layer in range(num_layer):
            if gnn_type == "gin":
                self.gnns.append(GINConvComplete(emb_dim, aggr="mean"))
            elif gnn_type == "gcn":
                self.gnns.append(GCNConv(emb_dim, aggr="mean"))
...
# class Multi_Motif_Fusion(nn.Module):
#     def __init__(self, emb_dim):
#         super(Multi_Motif_Fusion, self).__init__()
...
#         w = self.project(reprs).mean(0)                    # (M, 1)
...
class Multi_Motif_Fusion(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        new_mask = mask.expand(-1, -1, mask.shape[1]).transpose(1,2)

        score = score.masked_fill(new_mask==0, -1000000)  
        attn = F.softmax(score, -1)
        output = (mask * attn) @ V   
        
        return output.sum(1), attn.mean(0)[0,:]
