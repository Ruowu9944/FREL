    ...
    def process(self):
        ...
        if self.dataset == 'zinc_standard_agent':
            ...
            for i in range(len(smiles_list)):
                ...
                # the dataset
                data.input_ids = input_ids[i].unsqueeze(0)
                data.mask = mask[i].unsqueeze(0)
                data.y = torch.tensor(labels[i, :])
                data_list.append(data)
                data_smiles_list.append(smiles_list[i])
            ...
            for i in range(len(smiles_list)):
                ...
                # ========= manually add mol roberta_tensor =========
                data.input_ids = input_ids[i].unsqueeze(0)
                data.mask = mask[i].unsqueeze(0)
                # ===================================================================
                data.y = torch.tensor([labels[i]])
            ...
            for i in range(len(smiles_list)):
                ...
                if rdkit_mol != None:
                    ...
                    # manually add mol id
                    data.id = torch.tensor(
                        [j])  # id here is the index of the mol in
                    # the dataset
                    data.input_ids = input_ids[j].unsqueeze(0)
                    data.mask = mask[j].unsqueeze(0)
                    data.y = torch.tensor([labels[i]])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
                    j+=1
