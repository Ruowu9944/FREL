...
class MaskAtom:
    ...
    def __call__(self, data, masked_atom_indices=None):
        ...
        * mask rate number of atom indices
        ...
        # ----------- graphMAE -----------
        # if data.mask_node_label[:, 0] >= self.num_atom_type: pdb.set_trace()
...
class BatchMasking(Data):
    ...
    def from_data_list(data_list):
        ...
        for key in keys:
            batch[key] = []
        batch.batch = []
        if 'motif_batch' in data_list[0].keys: batch.motif_batch = []
        ...
        for i, data in enumerate(data_list):
            ...
            for key in data.keys:
                ...
                elif key == 'motif_batch':
                    item = item + cumsum_motif
                batch[key].append(item)

            cumsum_node += num_nodes
            cumsum_edge += data.edge_index.shape[1]
            if 'motif_batch' in data.keys: cumsum_motif += num_motif
